...
struct ZdcQVectors {
  ...
  // Histogram container
  struct HistSet {
    std::shared_ptr<TProfile> cent;
    std::shared_ptr<TProfile> vx;
    std::shared_ptr<TProfile> vy;
    std::shared_ptr<TProfile> vz;
  };

  // [iteration][step][component]
  std::vector<std::vector<std::array<HistSet, 4>>> hQ;
  std::vector<std::vector<std::array<HistSet, 4>>> hQcorr; 
  ...
  void init(InitContext const&)
  {
    ...
    if (!cfgFillNothing) {
      if (cfgFillHistRegistry) {
        ...
        // Added
        registry.add<TH2>("QA/ZNA_sumPM1-4_vs_Common", "ZNA_sumPM1-4_vs_Common", kTH2D, {{300, 0, 300}, {300, 0, 300}});
        registry.add<TH2>("QA/ZNC_sumPM1-4_vs_Common", "ZNC_sumPM1-4_vs_Common", kTH2D, {{300, 0, 300}, {300, 0, 300}});
        registry.add<TH2>("QA/ZNA_sumPM1-4_over_Common_vs_Common", "ZNA_sumPM1-4_over_Common_vs_Common", kTH2D, {{300, 0, 300}, {300, 0.5, 1.5}});
        registry.add<TH2>("QA/ZNC_sumPM1-4_over_Common_vs_Common", "ZNC_sumPM1-4_over_Common_vs_Common", kTH2D, {{300, 0, 300}, {300, 0.5, 1.5}});

        registry.add<TProfile>("QA/ZNA_Qx_noEq", "ZNA_Qx_noEq", kTProfile, {{1, 0, 1.}});
        registry.add<TProfile>("QA/ZNA_Qy_noEq", "ZNA_Qy_noEq", kTProfile, {{1, 0, 1.}});
        registry.add<TProfile>("QA/ZNC_Qx_noEq", "ZNC_Qx_noEq", kTProfile, {{1, 0, 1.}});
        registry.add<TProfile>("QA/ZNC_Qy_noEq", "ZNC_Qy_noEq", kTProfile, {{1, 0, 1.}});

        // Add histograms per recentering iteration
        // Make sure number of iterations matches
        int nIterations = 5;

        // Only one step per iteration for now
        int nSteps = 1;

        // Resize hQ properly: [iteration][step][component]
        hQ.resize(nIterations);
        hQcorr.resize(nIterations);
        for (int it = 0; it < nIterations; it++) {
          hQ[it].resize(nSteps);
          hQcorr[it].resize(nSteps);
        }

        // Component names
        const char* compNames[4] = {"QXA", "QYA", "QXC", "QYC"};

        // Declaring histograms for iterations only, so 1 step
        for (int it = 1; it <= nIterations; it++) {
          int step = 1; // fixed step
          for (int c = 0; c < 4; c++) {
            auto& h = hQ[it - 1][step - 1][c];
            auto& hCorr = hQcorr[it - 1][step - 1][c];

            h.cent = registry.add<TProfile>(
              Form("QA/it%d_step%d/h%s_vs_cent", it, step, compNames[c]),
              Form("%s vs centrality;Centrality (%%);%s", compNames[c], compNames[c]),
              {HistType::kTProfile, {axisCent}});

            h.vx = registry.add<TProfile>(
              Form("QA/it%d_step%d/h%s_vs_vx", it, step, compNames[c]),
              Form("%s vs vertex x;vertex x (cm);%s", compNames[c], compNames[c]),
              {HistType::kTProfile, {axisVx}});

            h.vy = registry.add<TProfile>(
              Form("QA/it%d_step%d/h%s_vs_vy", it, step, compNames[c]),
              Form("%s vs vertex y;vertex y (cm);%s", compNames[c], compNames[c]),
              {HistType::kTProfile, {axisVy}});

            h.vz = registry.add<TProfile>(
              Form("QA/it%d_step%d/h%s_vs_vz", it, step, compNames[c]),
              Form("%s vs vertex z;vertex z (cm);%s", compNames[c], compNames[c]),
              {HistType::kTProfile, {axisVz}});
          
            //For corrections to Q vectors
            hCorr.cent = registry.add<TProfile>(
              Form("QA/it%d_step%d/h%s_corr_vs_cent", it, step, compNames[c]),
              Form("%s correction vs centrality;Centrality (%%);%s correction", compNames[c], compNames[c]),
              {HistType::kTProfile, {axisCent}});

            hCorr.vx = registry.add<TProfile>(
              Form("QA/it%d_step%d/h%s_corr_vs_vx", it, step, compNames[c]),
              Form("%s correction vs vertex x;vertex x (cm);%s correction", compNames[c], compNames[c]),
              {HistType::kTProfile, {axisVx}});

            hCorr.vy = registry.add<TProfile>(
              Form("QA/it%d_step%d/h%s_corr_vs_vy", it, step, compNames[c]),
              Form("%s correction vs vertex y;vertex y (cm);%s correction", compNames[c], compNames[c]),
              {HistType::kTProfile, {axisVy}});

            hCorr.vz = registry.add<TProfile>(
              Form("QA/it%d_step%d/h%s_corr_vs_vz", it, step, compNames[c]),
              Form("%s correction vs vertex z;vertex z (cm);%s correction", compNames[c], compNames[c]),
              {HistType::kTProfile, {axisVz}});
  ...
  // Added for iteration and step histograms for recentering
  inline void fillqStep(int it, int step, const std::array<double, 4>& q, const std::array<double, 4>& qCorr, const std::vector<float>& v, double centrality)
  {
    for (int c = 0; c < 4; c++) {
      auto& h = hQ[it - 1][step - 1][c];
      auto& hCorr = hQcorr[it - 1][step - 1][c];

      h.cent->Fill(centrality, q[c]);
      h.vx->Fill(v[0], q[c]);
      h.vy->Fill(v[1], q[c]);
      h.vz->Fill(v[2], q[c]);

      // Corrections to the Q vectors
      hCorr.cent->Fill(centrality, qCorr[c]);
      hCorr.vx->Fill(v[0], qCorr[c]);
      hCorr.vy->Fill(v[1], qCorr[c]);
      hCorr.vz->Fill(v[2], qCorr[c]);
    }
  }
               ...
               UsedTracks const& tracks)
  {
    ...
    if (!foundBC.has_zdc()) {
      cal.isSelected = false;
      spTableZDC(runnumber, cents, v, foundBC.timestamp(), 0, 0, 0, 0, cal.isSelected, 0, UndefValueFloat, UndefValueFloat, UndefValueFloat, UndefValueFloat, UndefValueFloat, UndefValueFloat, UndefValueFloat, UndefValueFloat, UndefValueFloat, UndefValueFloat);
    ...
    if (!isZNAhit || !isZNChit) {
      cal.isSelected = false;
      spTableZDC(runnumber, cents, v, foundBC.timestamp(), 0, 0, 0, 0, cal.isSelected, 0, eZN[0], eZN[1], eZN[2], eZN[3], eZNA, eZN[4], eZN[5], eZN[6], eZN[7], eZNC);
      cal.lastRunNumber = runnumber;
      return;
    }
    ...
    if (cent < EvSel.cfgCentMin || cent > EvSel.cfgCentMax || std::abs(collision.posZ()) > cfgVtxZ || !collision.sel8()) {
      // event not selected
      cal.isSelected = false;
      spTableZDC(runnumber, cents, v, foundBC.timestamp(), 0, 0, 0, 0, cal.isSelected, eventSelectionFlags, eZN[0], eZN[1], eZN[2], eZN[3], eZNA, eZN[4], eZN[5], eZN[6], eZN[7], eZNC);
      cal.lastRunNumber = runnumber;
      return;
    }
    ...
    if (cfgFillHistRegistry && !cfgFillNothing && isEventSelected) {
      for (int i = 0; i < nTowersPerSide; i++) {
        ...
        registry.get<TProfile>(HIST("QA/ZNA_pmC"))->Fill(Form("%d", runnumber), meanEZN[0]);
        registry.get<TProfile>(HIST("QA/ZNA_pm1"))->Fill(Form("%d", runnumber), eZN[0]);
        registry.get<TProfile>(HIST("QA/ZNA_pm2"))->Fill(Form("%d", runnumber), eZN[1]);
        registry.get<TProfile>(HIST("QA/ZNA_pm3"))->Fill(Form("%d", runnumber), eZN[2]);
        registry.get<TProfile>(HIST("QA/ZNA_pm4"))->Fill(Form("%d", runnumber), eZN[3]);

        registry.get<TProfile>(HIST("QA/ZNC_pmC"))->Fill(Form("%d", runnumber), meanEZN[5]);
        registry.get<TProfile>(HIST("QA/ZNC_pm1"))->Fill(Form("%d", runnumber), eZN[4]);
        registry.get<TProfile>(HIST("QA/ZNC_pm2"))->Fill(Form("%d", runnumber), eZN[5]);
        registry.get<TProfile>(HIST("QA/ZNC_pm3"))->Fill(Form("%d", runnumber), eZN[6]);
        registry.get<TProfile>(HIST("QA/ZNC_pm4"))->Fill(Form("%d", runnumber), eZN[7]);
        ...
        registry.fill(HIST("QA/ZNA_pm1_vs_Centrality"), cent, eZN[0] / sumZNA);
        registry.fill(HIST("QA/ZNA_pm2_vs_Centrality"), cent, eZN[1] / sumZNA);
        registry.fill(HIST("QA/ZNA_pm3_vs_Centrality"), cent, eZN[2] / sumZNA);
        registry.fill(HIST("QA/ZNA_pm4_vs_Centrality"), cent, eZN[3] / sumZNA);
        ...
        registry.fill(HIST("QA/ZNC_pm1_vs_Centrality"), cent, eZN[4] / sumZNC);
        registry.fill(HIST("QA/ZNC_pm2_vs_Centrality"), cent, eZN[5] / sumZNC);
        registry.fill(HIST("QA/ZNC_pm3_vs_Centrality"), cent, eZN[6] / sumZNC);
        registry.fill(HIST("QA/ZNC_pm4_vs_Centrality"), cent, eZN[7] / sumZNC);
        ...
        // Added: compare sum of PMTs 1-4 with common energy
        // 2D correlation plots with common energy on x-axis and sum of PMTs 1-4 on y-axis
        registry.fill(HIST("QA/ZNA_sumPM1-4_vs_Common"), zdcCol.energyCommonZNA(), sumZNA);
        registry.fill(HIST("QA/ZNC_sumPM1-4_vs_Common"), zdcCol.energyCommonZNC(), sumZNC);
        registry.fill(HIST("QA/ZNA_sumPM1-4_vs_Common"), zdcCol.energyCommonZNA(), sumZNA);
        registry.fill(HIST("QA/ZNC_sumPM1-4_vs_Common"), zdcCol.energyCommonZNC(), sumZNC);
        // 1D profiles of ratio of sum PMTs 1-4 over common energy vs common energy
        registry.fill(HIST("QA/ZNA_sumPM1-4_over_Common_vs_Common"), zdcCol.energyCommonZNA(), sumZNA / zdcCol.energyCommonZNA());
        registry.fill(HIST("QA/ZNC_sumPM1-4_over_Common_vs_Common"), zdcCol.energyCommonZNC(), sumZNC / zdcCol.energyCommonZNC());
        //registry.fill(HIST("QA/ZNA_sumPM1-4_over_Common_vs_Common"), zdcCol.energyCommonZNA(), sumZNA / zdcCol.energyCommonZNA());
        //registry.fill(HIST("QA/ZNC_sumPM1-4_over_Common_vs_Common"), zdcCol.energyCommonZNC(), sumZNC / zdcCol.energyCommonZNC());
      }
    }
    ...
    if (cal.atIteration == 0) {
      ...
      spTableZDC(runnumber, cents, v, foundBC.timestamp(), q[0], q[1], q[2], q[3], cal.isSelected, eventSelectionFlags, eZN[0], eZN[1], eZN[2], eZN[3], eZNA, eZN[4], eZN[5], eZN[6], eZN[7], eZNC);
      cal.lastRunNumber = runnumber;
      return;
    } else {
      ...
      // Extra q vectors for every iteration
      std::array<double, 4> qStep(q); // working Q-vector
      std::array<double, 4> corrqStep{};

      for (int it = 1; it <= nIterations; it++) {
        // Cumulative correction across all steps in this iteration
        corrqStep = {0.0, 0.0, 0.0, 0.0};
        ...
        qStep[0] -= corrQxA.back();
        qStep[1] -= corrQyA.back();
        qStep[2] -= corrQxC.back();
        qStep[3] -= corrQyC.back();

        corrqStep[0] += corrQxA.back();
        corrqStep[1] += corrQyA.back();
        corrqStep[2] += corrQxC.back();
        corrqStep[3] += corrQyC.back();
        ...
        fillqStep(it, 1, qStep, corrqStep, v, cent);
      ...
      spTableZDC(runnumber, cents, v, foundBC.timestamp(), qXaShift, qYaShift, qXcShift, qYcShift, cal.isSelected, eventSelectionFlags, eZN[0], eZN[1], eZN[2], eZN[3], eZNA, eZN[4], eZN[5], eZN[6], eZN[7], eZNC);
      qRec = {0, 0, 0, 0};
